...
function closestEdge(bounds, target) {
  ...
  var isTopOrBottom = verticalRelative ? 'top' : 'bottom';

  var isLeftOrRight = horizontalRelative ? 'left' : 'right';

  var a = verticalRelative ? (1 - position.vertical) : position.vertical;

  var b = horizontalRelative ? (1 - position.horizontal) : position.horizontal;

  return a <= b ? isTopOrBottom : isLeftOrRight;
}
