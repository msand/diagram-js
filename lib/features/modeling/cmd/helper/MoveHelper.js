...
MoveHelper.prototype.moveClosure = function(closure, delta, newParent, newHost) {
  ...
  // TODO (nre): Ugly hack, remove!
  var isMultiMove = Object.keys(allShapes).length > 1;

  // move all shapes
  forEach(allShapes, function(s) {

    // update the attachment of top-level elements
    //
    // if null was passed as the newHost we assume the element
    // is being "detached" us the ability to remove
    // the previous attachment
    if (!isMultiMove && newHost !== undefined && topLevel[s.id]) {
      modeling.attachShape(s, newHost);
    }
